...
T_exp getFunLink(Tr_level dec_level,Tr_level use_level){   //let语句中，声明的函数比原函数高一层，所以调用函数的话，需要用parent比，这bug找了半天，特记之
   ...
   while(dec_level->parent != use_level){
     fp = T_Mem(T_Binop(T_plus,T_Const(2*F_wordSize),fp));
...
Tr_exp Tr_callExp(Temp_label name, Tr_level dec, Tr_level use,Tr_expList args)
{
   ...
   if(staticLink != NULL)
      ...
      return Tr_Ex(T_Call(T_Name(name),targs));
}
...
Tr_exp Tr_opExp(A_oper oper, Tr_exp left, Tr_exp right)
{
   ...
   switch(oper){
     ...
     {
        T_stm stm;
        if(unEx(left)->kind == T_NAME || unEx(right)->kind == T_NAME){
              T_expList list = T_ExpList(unEx(left),T_ExpList(unEx(right),NULL));
              stm = T_Cjump(relop,F_externalCall("stringEqual",list),T_Const(1),NULL,NULL);
        }
        else
          stm = T_Cjump(relop,unEx(left),unEx(right),NULL,NULL);
...
Tr_exp Tr_recordExp(int count, Tr_expList paras)
{
   ...
   T_stm seq = T_Move(T_Temp(temp),F_externalCall("allocRecord",size));  
   T_stm stm = NULL;
   int i = 0;
   while(tparas != NULL){
     if(stm == NULL)
...
Tr_exp Tr_ifExp(Tr_exp ifexp, Tr_exp thenexp, Tr_exp elseexp)
{
   ...
   else
   {
      if(thenexp->kind == Tr_ex && elseexp->kind == Tr_ex)
      {
         ...
         return Tr_Ex(T_Eseq(cx.stm,T_Eseq(T_Label(t),T_Eseq(T_Move(T_Temp(r),tvalue),T_Eseq(T_Jump(T_Name(conbine),Temp_LabelList(conbine,NULL)),T_Eseq(T_Label(f),T_Eseq(T_Move(T_Temp(r),fvalue),T_Eseq(T_Label(conbine),T_Temp(r)))))))));
...
Tr_exp Tr_funDec(Tr_level level, Tr_exp body) {
    T_stm bodystm = T_Move(T_Temp(F_RV()),unEx(body));
...
void Tr_addOutermost(Tr_exp exp){
     fragList = F_FragList(F_ProcFrag(unNx(exp),Tr_outermost()->frame),fragList);
}
